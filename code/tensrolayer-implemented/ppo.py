...
class PPO(object):
    ...
    def a_train(self, tfs, tfa, tfadv):
        ...
        tfadv = np.array(tfadv, np.float32) #优势函数 -值函数的变形
        with tf.GradientTape() as tape:
            mu, sigma = self.actor(tfs)
            pi = tfp.distributions.Normal(mu, sigma) #初始化策略 正态分布
            ...
            surr = ratio * tfadv
            if METHOD['name'] == 'kl_pen': #使用KL作为惩罚项的loss
                tflam = METHOD['lam']
                kl = tfp.distributions.kl_divergence(oldpi, pi)
                kl_mean = tf.reduce_mean(kl)
                aloss = -(tf.reduce_mean(surr - tflam * kl))
            else:  # clipping method, find this is better 使用clip作为loss
    ...
    def update_old_pi(self):
        ...
        for p, oldp in zip(self.actor.trainable_weights, self.actor_old.trainable_weights):
            oldp.assign(p) #将新的pi的参数直接赋值给旧的pi

    def c_train(self, tfdc_r, s): #训练critic网络，mse优化
        '''
    ...
    def update(self, s, a, r):
        ...
        for _ in range(C_UPDATE_STEPS):
            self.c_train(r, s) #Critic的训练一次更新十步
    ...
    if args.train:
        ...
        for ep in range(EP_MAX):
            ...
            for t in range(EP_LEN):  # in one episode
                ...
                # update ppo
                if (t + 1) % BATCH == 0 or t == EP_LEN - 1: #采集了一个Batch的数据或者走到最后一步了才进行更新
                    v_s_ = ppo.get_v(s_)
                    discounted_r = []
                    for r in buffer_r[::-1]:
                        v_s_ = r + GAMMA * v_s_

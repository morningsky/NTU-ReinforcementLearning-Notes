...
class Actor(object):
    def __init__(self, sess, action_dim, action_bound, learning_rate, replacement):
        ...
        with tf.variable_scope('Actor'):
            # 这个网络用于及时更新参数
            self.a = self._build_net(S, scope='eval_net', trainable=True) #由target网络给出确定的action
            #对比ppo 网络的输出是一个概率分布 
    ...
    def choose_action(self, s):
        ...
        #对比ppo a = self.sess.run(self.sample_op, {self.tfs:s})[0]
    ...
    def add_grad_to_graph(self, a_grads):
        with tf.variable_scope('policy_grads'):
            ...
            self.policy_grads = tf.gradients(ys=self.a, xs=self.e_params, grad_ys=a_grads) ##grad_ys 这是从 Critic 来的 dQ/da
...
class Critic(object):
    def __init__(self, sess, state_dim, action_dim, learning_rate, gamma, replacement, a, a_):
        ...
        with tf.variable_scope('target_q'):
            self.target_q = R + self.gamma * self.q_ ## self.q_ 根据 Actor 的 target_net 来
